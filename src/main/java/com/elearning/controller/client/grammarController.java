...
public class grammarController {
	...
	/*
	 * @Autowired CommentGrammarService commentgrammarService;
	 */
	@Autowired
...
//		model.addAttribute("listData", list);
	 ...
	 * "/grammar/ajaxCmtGram/{contentComment}/{baiGrammarId}", method =
	 * RequestMethod.POST)
	 * 
	 * @ResponseBody public List<String> getAjax(@PathVariable("contentComment")
	 ...
	 * 
	 * BaiGrammar baigrammar = baigrammarService.getBaiGrammar(id).get(0); Date time
	 * = new Date();
	 * 
	 * // save comment CommentGrammar cmt = new CommentGrammar();
	 * cmt.setBaigrammar(baigrammar); cmt.setNguoidung(currentUser);
	 * cmt.setNgaycomment(time); cmt.setCmtgrammarcontent(contentComment);
	 * commentgrammarService.save(cmt);
	 * 
	 * response.add(cmt.toString()); return response;
	 * 
	 * }
	 */
